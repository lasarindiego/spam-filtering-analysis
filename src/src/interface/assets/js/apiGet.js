    ...
    .then((response)=>{
        ...
        //Antideo
            ...
            if(response.data.hasOwnProperty('ipHealth')){
                document.getElementById('proxy').insertAdjacentHTML(`afterbegin`,`${response.data.ipHealth.hasOwnProperty('proxy') ? 'Proxy: ' +  response.data.ipHealth.proxy : 'Não foi possível verificar o Proxy'}`);
        ...
        const phishTanking = document.querySelectorAll('[id="phishTank"]');
        phishTanking ? phishTanking.forEach((item) => item.remove()) : undefined;
        response.data.phish.map((item, index) => {
            if(item.database == 'Sim'){
                document.getElementById(`firstTdP`).insertAdjacentHTML(`beforeend`, `<div id="phishTank"><br><table  class="alt"><thead><tr><th>Descrição</th><th>Resultado</th></tr><thead><tbody></tr><tr><td colspan='2' id="phish${index}u"><b>URL: ${item.url}</b></td></tr><tr><td>Foi possível realizar a checagem?</td><td id="phish${index}d">${item.database}</td></tr><tr><td id="phish${index}va"> O site é um phishing? </td><td>${item.valid}</td></tr><tr><td id="phish${index}ve">Foi verificado?</td><td> ${item.verified}</td></tr><tr><td id="phish${index}at>Verificado em</td><td> ${item.verified_at}</td></tr></tbody></table></div>`);
            }else{
                document.getElementById(`firstTdP`).insertAdjacentHTML(`beforeend`, `<div id="phishTank"><br><table  class="alt"><thead><tr><th>Descrição</th><th>Resultado</th></tr><thead><tbody></tr><tr><td colspan='2' id="phish${index}u"><b>URL: ${item.url}</b></td></tr><tr><td>Foi possível realizar a checagem?</td><td id="phish${index}d">${item.database}</td></tr></tbody></table></div>`);
